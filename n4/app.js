...
function entryRowHtml(e) {
  ...
  const typeClass = e.type === "correction" ? " entry--correction" : e.type === "priority" ? " entry--priority" : e.type === "status" ? " entry--status" : e.type === "new" ? " entry--new" : " entry--note";
  return `
...
if (window.__loginState && window.__loginState.loggedIn) {
  ...
  window.addEventListener("n4-login-success", onLoginSuccess, { once: true });
}
